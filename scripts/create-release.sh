...
GITHUB_REPO="Jmi2020/claude-nook"

# Website repo for auto-updating appcast
WEBSITE_DIR="${CLAUDE_NOOK_WEBSITE:-$PROJECT_DIR/../ClaudeNook-website}"
WEBSITE_PUBLIC="$WEBSITE_DIR/public"

APP_PATH="$EXPORT_PATH/Claude Nook.app"
APP_NAME="ClaudeNook"
KEYCHAIN_PROFILE="ClaudeNook"
...
if command -v create-dmg &> /dev/null; then
    ...
    create-dmg \
        ...
        --window-size 600 400 \
        --icon-size 100 \
        --icon "Claude Nook.app" 150 200 \
        --app-drop-link 450 200 \
        --hide-extension "Claude Nook.app" \
        "$DMG_PATH" \
        "$APP_PATH"
else
    echo "Using hdiutil (install create-dmg for prettier DMG: brew install create-dmg)"
    hdiutil create -volname "Claude Nook" \
        -srcfolder "$APP_PATH" \
        -ov -format UDZO \
        "$DMG_PATH"
fi
...
POSSIBLE_PATHS=(
    "$HOME/Library/Developer/Xcode/DerivedData/ClaudeNook-*/SourcePackages/artifacts/sparkle/Sparkle/bin"
)
...
if ! command -v gh &> /dev/null; then
    ...
    if gh release view "v$VERSION" --repo "$GITHUB_REPO" &>/dev/null; then
        ...
        gh release create "v$VERSION" "$DMG_PATH" \
            --repo "$GITHUB_REPO" \
            --title "Claude Nook v$VERSION" \
            --notes "## Claude Nook v$VERSION

### Installation
1. Download \`$APP_NAME-$VERSION.dmg\`
2. Open the DMG and drag Claude Nook to Applications
3. Launch Claude Nook from Applications

### Auto-updates
After installation, Claude Nook will automatically check for updates."
    fi
