#!/bin/bash
#
# Test Claude Island TCP connection
...
# Load configuration
CONFIG_FILE="$HOME/.config/claude-island/claude-island.env"
...
HOST="${CLAUDE_ISLAND_HOST:-127.0.0.1}"
PORT="${CLAUDE_ISLAND_PORT:-4851}"
TOKEN="${CLAUDE_ISLAND_TOKEN:-}"
SOCKET_PATH="/tmp/claude-island.sock"

echo "=== Claude Island Connection Test ==="
echo ""
echo "Configuration:"
echo "  Host:   $HOST"
echo "  Port:   $PORT"
...
if [[ -S "$SOCKET_PATH" ]]; then
    echo "   [OK] Socket exists - Claude Island is running locally"
else
...
if [[ "$TCP_OK" == "true" ]]; then
    ...
    echo "   - Is Claude Island running on your Mac?"
    echo "   - Is TCP enabled in Claude Island settings?"
...
if [[ -z "$TOKEN" ]]; then
    echo "   [SKIP] No token configured (CLAUDE_ISLAND_TOKEN not set)"
    exit 0
fi
...
if [[ "$AUTH_RESPONSE" == "OK" ]]; then
    ...
    if [[ "$FULL_RESPONSE" == "OK"* ]]; then
        ...
        echo ""
        echo "   If Claude Island is visible, you should see a notification appear!"
    else
    ...
    echo ""
    echo "   Check that your token matches the one in Claude Island settings."
    exit 1
fi
...
echo ""
echo "Claude Island is reachable and authenticated at $HOST:$PORT"
